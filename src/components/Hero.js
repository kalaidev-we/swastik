  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        ...
        />
        {/* Light concrete gradient overlay (Fades left to right from light off-white to transparent) */}
        <div className="absolute inset-y-0 left-0 w-full lg:w-[65%] bg-gradient-to-r from-bg-main via-bg-main/90 to-transparent z-10" />
        {/* Soft bottom lift */}
        <div className="absolute inset-x-0 bottom-0 h-40 bg-gradient-to-t from-bg-main to-transparent z-10" />
        {/* Soft light overlay for image side (right) to keep text readable */}
      ...
      <div className="relative z-20 max-w-[1440px] mx-auto px-6 md:px-12 w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-center py-16 md:py-24">
        ...
        <div className="lg:col-span-7 flex flex-col items-start text-left">
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white border border-black/5 shadow-sm backdrop-blur-md mb-6"
          >
          ...
          >
            <button
              ...
              className="flex items-center gap-2 px-8 py-3.5 rounded-full bg-accent-gold hover:bg-accent-gold/90 text-white font-mono font-bold uppercase tracking-widest transition-all duration-300 shadow-[0_4px_14px_rgba(197,168,128,0.3)] clickable group"
            >
              <span>Explore Products</span>
              <ArrowRight size={14} className="transform group-hover:translate-x-1 transition-transform text-white" />
            </button>

            <button
              onClick={openWhatsApp}
              className="flex items-center gap-2 px-8 py-3.5 rounded-full bg-white hover:bg-bg-main text-text-main border border-black/5 font-mono font-bold uppercase tracking-widest transition-all duration-300 shadow-sm clickable"
            >
        ...
        {/* Hero Right: Floating Stats Cards (Polished Aluminum) */}
          ...
          {stats.map((stat, index) => {
            ...
            return (
              <motion.div
                ...
                className="steel-embossed p-6 rounded-2xl flex flex-col justify-between min-h-[140px] md:min-h-[160px] relative overflow-hidden group shadow-md hover:border-accent-gold/30 transition-all duration-300 select-none"
              >
                ...
                {/* Corner rivets */}
                <span className="rivet absolute top-1.5 left-1.5 animate-pulse" />
